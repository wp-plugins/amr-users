...
== Description ==
This plugin is aimed at helping developers who need to analys, slice and dice user data without coding a whole application.  It can also be used by non-coders to pull together various plugins and present member data cleanly for small to medium sites. The ability to include, exclude and sort user data generated by a variety of plugins is very useful.

Features:

*  Highly configurable user listings by meta keys and values, and additional computed fields, comment count and post count.
 
*  Includes User Search, bulk actions like delete, configurable action links, display, inclusion, exclusion, sorting configuration and an option to export to CSV. 
 
*  Make some lists public to use in with a shortcode.  Please suitably define the fields, lists and protection for the shortcode to manage your users privacy.
...
*  Cacheing is used to improve the response for large user sites.  Cache can be updated on update of user records, or by cron job, or on manual request. 

*  Pre-configured examples to get you started - you can add more or change these. You must have some data to see the fields.  In the Screenshots you may see data from subscribe 2, register plus and your members plugins.

*  Export and import settings from other systems

*  Show any list in a widget - make it simple please!

More information: [user list plugin website](http://wpusersplugin.com/).
...
Note:If you have a extremely high user volume with high updates, a custom written, lean, mean solution may be more appropriate than this general solution.  That said there is cacheing of user data and wp transient cacheing of the html generated to minimise performance load of extracting data from multiple sources, particularly for those on small shared hosts.  Set your sytem up carefully or rebuild the cache on request only.
